...
Ключевые фрагменты сценария настройки приведены в листингах~\ref{lst:p1-dirs} и~\ref{lst:p1-files}. Проверка исполнения файла~\mdash  листинг~\ref{lst:p1-exec}; удаление существующих \texttt{file*} с восстановлением~\mdash  листинг~\ref{lst:p1-rm}.
...
\begin{lstlisting}[caption={Исполнение файла: реальный запуск, не проверка бита x},label={lst:p1-exec}]
check_exec() {
  local user="$1" path="$2"
  if [[ ${user} == root ]]; then
    timeout 1s bash -c 'printf "\n" | "$1"' _ "${path}"
  else
    timeout 1s runuser -u "${user}" -- \
      bash -c 'printf "\n" | "$1"' _ "${path}"
  fi
}
\end{lstlisting}

\begin{lstlisting}[caption={Удаление существующих file* с восстановлением из копии},label={lst:p1-rm}]
cp -a -- "${files[@]}" "${bak}/"
failed=0
for f in "${files[@]}"; do
  if [[ ${user} == root ]]; then
    ...
    runuser -u "${user}" -- rm -f -- "${f}" || failed=1
  fi
done
cp -a -- "${bak}/." "${dpath}/"
\end{lstlisting}
