...
### Анализ пропущенных значений и их обработка (удаление)
...
# Удаление колонок с пропусками (если доля > 50%)
threshold = 0.5
df_no_missing_cols = df.dropna(axis=1, thresh=int(len(df) * (1 - threshold)))

print("\nРазмер после удаления строк:", df_no_missing_rows.shape)
print("Размер после удаления колонок:", df_no_missing_cols.shape)

### Обнаружение и обработка выбросов
# Выбор числовых колонок
numeric_cols = ['age', 'bmi', 'systolic_bp', 'ldl', 'income', 'annual_medical_cost']
...
# Применяем обрезку к колонкам с выбросами (например, income и bmi)
cols_to_cap = ['income', 'bmi']
...
one_hot_cols = ['sex', 'region', 'urban_rural', 'plan_type', 'network_tier']
encoder_one_hot = OneHotEncoder(drop='first', sparse_output=False)
...
# Label Encoding для упорядоченных переменных
label_cols = ['education', 'marital_status', 'employment_status', 'smoker', 'alcohol_freq']
encoder_label = LabelEncoder()
for col in label_cols:
    # Проверяем на NaN и заменяем их модой перед кодированием
    if df[col].isnull().sum() > 0:
        df[col] = df[col].fillna(df[col].mode()[0])
    df[col] = encoder_label.fit_transform(df[col].astype(str))

# Объединяем закодированные данные с исходным датасетом
df_encoded = pd.concat([df.drop(categorical_cols, axis=1), encoded_df_one_hot], axis=1)
...
# Определяем числовые колонки (исключаем бинарные и закодированные)
numeric_cols_all = [col for col in df_encoded.columns if col not in ['is_high_risk', 'had_major_procedure',
                                                                   'hypertension', 'diabetes', 'asthma', 'copd',
                                                                   'cardiovascular_disease', 'cancer_history',
                                                                   'kidney_disease', 'liver_disease', 'arthritis',
                                                                   'mental_health'] + list(encoded_df_one_hot.columns)]
...
for i, col in enumerate(numeric_cols_all[:6], 1):
    plt.subplot(2, 3, i)
...
y = df_encoded['is_high_risk']  # Целевая переменная
...
recall_dt = recall_score(y_test, y_test)
