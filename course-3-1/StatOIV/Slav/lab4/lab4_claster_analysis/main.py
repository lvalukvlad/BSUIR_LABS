...
target_col = "OverallQual"  # Используем OverallQual как категориальные "метки" (от 1 до 10)
print(f"Целевая переменная (для ARI/NMI): {target_col}")
print(f"Количество различных значений: {df[target_col].nunique()}")
...
candidate_features = [
    "LotArea", "YearBuilt", "GrLivArea", "TotalBsmtSF",
...
dendrogram(linkage_matrix, truncate_mode='lastp',  # Показывать только последние p объединений
...
plt.axhline(y=eps_95, color='orange', linestyle='--', linewidth=1.5, label=f'95-й процентиль: {eps_95:.4f}')
plt.axhline(y=eps_98, color='red', linestyle='--', linewidth=1.5, label=f'98-й процентиль: {eps_98:.4f}')
plt.legend()
plt.tight_layout()
...
if max(kmeans_ari, agg_ari, db_ari) < 0.3:
    print(f"\nНизкие значения ARI/NMI указывают на то, что найденные кластеры не совпадают с истинными метками.")
...
print(f" Для данного датасета лучше всего подходит алгоритм {best_sil_alg} с точки зрения внутренних метрик качества кластеризации.")
...
print("Для данного датасета, оба метода (K-Means/ward) согласуются, но DBSCAN чувствителен к eps, допускает много выбросов и зачастую находит меньшее число крупных кластеров или много шума.")
...
# Вывод
print("Итоги")
print(f""" Анализ завершён! Результаты по качеству:
