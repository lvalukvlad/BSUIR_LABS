...
DECLARE
    new_id INT;
BEGIN
    INSERT INTO Document (Number, CreationDate, ReleaseDate, Content, DocumentType, SignerPosition, SignerFullName)
    VALUES (p_number, p_creation_date, p_release_date, p_content, p_document_type, p_signer_position, p_signer_full_name)
...
BEGIN
    ...
    WHERE d.DocumentType = p_document_type
      AND d.CreationDate <= CURRENT_DATE
    ORDER BY d.CreationDate DESC, d.Number;
END;
$$;

-- 11. Триггерная функция для автоматического обновления статуса выполнения мероприятия
...
-- 13. Триггерная функция для проверки дат документа
