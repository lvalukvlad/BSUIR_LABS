...
class AgentOrchestrator:
    ...
    async def process_conversation(self, conversation_id: str, user_input: str) -> Dict[str, Any]:
        ...
        logger.info(f"Conversation {conversation_id} stage: {context.current_stage}")
        logger.info(f"   Symptoms: {len(context.symptom_texts)}")
        logger.info(f"   Has diagnosis: {context.primary_diagnosis is not None}")
    ...
    async def _handle_symptom_collection(self, context: ConversationContext, user_input: str) -> Dict[str, Any]:
        ...
        if len(context.symptom_texts) >= 1:
            context.current_stage = ConversationStage.DIAGNOSIS
        ...
        try:
            if "diagnostician" in self.agents:
                ...
                if diagnosis_result and "preliminary_diagnosis" in diagnosis_result:
                    ...
                    logger.info(f"Diagnosis saved: {diagnosis_info.name} (confidence: {confidence})")
            ...
            logger.warning("   Diagnostician failed, using fallback")
    ...
    async def _handle_treatment_fallback(self, context: ConversationContext) -> Dict[str, Any]:
        ...
        if "горло" in symptoms_text and "температура" in symptoms_text:
            diagnosis_name = "Фарингит"
            confidence = 0.7
        elif "кашель" in symptoms_text and "температура" in symptoms_text:
            diagnosis_name = "Бронхит"
            confidence = 0.7
        elif "температура" in symptoms_text:
            diagnosis_name = "ОРВИ"
            confidence = 0.8
        else:
        ...
        if "medicine_provider" in self.agents:
            try:
                ...
                treatment_result = await self.agents["medicine_provider"].process(medicine_state)

                if treatment_result and treatment_result.get("recommendation_success", False):
                    response = f"🔍 **Диагноз:** {diagnosis_name}\n\n"
                    response += treatment_result.get("recommendations_text", "")
                ...
                logger.error(f"Fallback treatment failed: {e}")

        response = f"**Диагноз:** {diagnosis_name}\n\n"
        response += "**Рекомендации:**\n\n"
        response += "1. **Парацетамол**\n"
        response += "   *Дозировка:* 500 мг 3-4 раза в день\n"
        response += "   *Применение:* Для снижения температуры и боли\n\n"
        response += "2. **Обильное теплое питье**\n"
        response += "3. **Полоскание горла солевым раствором**\n\n"
        response += "**Для точного лечения обратитесь к врачу.**"
    ...
    async def _handle_treatment(self, context: ConversationContext) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning("   Could not generate treatment recommendations")
    ...
    async def _handle_unknown(self, context: ConversationContext) -> Dict[str, Any]:
        logger.warning(f" Handling UNKNOWN stage for conversation {context.conversation_id}")
