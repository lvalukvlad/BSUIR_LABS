    ...
    async def execute(self, context: Any) -> Dict[str, Any]:
        ...
        for attempt in range(self.retries + 1):
            try:
                ...
                # УБРАЛИ сложную логику, просто передаем контекст как есть
...
class ParallelWorkflowNode:
    """Узел для параллельного выполнения"""
    ...
    async def execute(self, context: Any) -> Dict[str, Any]:
        ...
        # Запускаем все узлы параллельно
...
class SequentialWorkflowNode:
    """Узел для последовательного выполнения с ветвлением"""
    ...
    async def execute(self, context: Any) -> Dict[str, Any]:
        """Последовательное выполнение с возможностью прерывания"""
        ...
        for node in self.nodes:
            ...
            # Если узел завершился неудачно, прерываем выполнение
            ...
            # Обновляем контекст результатами узла
...
class WorkflowEngine:
    ...
    # В методе execute_workflow исправим обработку контекста:
    ...
    async def execute_workflow(self,
        ...
        """Выполняет workflow с исправленной обработкой ошибок"""
        ...
        # Устанавливаем workflow_id в контекст если возможно
        ...
        try:
            ...
            for node in nodes:
                ...
                try:
                    # ВАЖНО: Проверяем что агент существует и может быть вызван
                    ...
                    if node_result.get("status") == "completed" and "result" in node_result:
                        # Сохраняем в результаты workflow
                        ...
                        if node_result["result"]:
                            try:
                                if hasattr(context, 'update'):
                                    ...
                                    # Для других типов - просто сохраняем результаты
                    ...
                    # Логируем время выполнения узла
    ...
    async def execute_with_breakpoints(self,
        ...
        """Выполняет workflow с точками останова"""
        ...
        for node in nodes:
            ...
            # Проверяем, является ли текущий узел точкой останова
