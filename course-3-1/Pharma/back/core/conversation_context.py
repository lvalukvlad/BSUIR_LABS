"""
Контекст разговора для медицинского ассистента
"""
...
# В conversation_context.py добавляем в ConversationContext:
@dataclass
class LocalizationInfo:
    """Информация о локализации симптомов"""
    ...
    depth: Optional[str] = None  # superficial/deep
...
# В ConversationContext добавляем:
...
class SymptomInfo:
    ...
    intensity: Optional[str] = None  # mild/moderate/severe
    duration: Optional[str] = None  # hours/days/weeks
    onset: Optional[str] = None  # sudden/gradual
    frequency: Optional[str] = None  # constant/intermittent
...
class ContraindicationInfo:
    """Информация о противопоказаниях пользователя"""
    allergies: List[str] = field(default_factory=list)
    chronic_conditions: List[str] = field(default_factory=list)
    current_medications: List[str] = field(default_factory=list)
    pregnancy: Optional[bool] = None
...
class ConversationContext:
    """Полный контекст медицинской консультации"""
    ...
    symptom_texts: List[str] = field(default_factory=list)  # Оригинальные тексты
    ...
    # В методе to_dict() класса ConversationContext добавьте/исправьте:

    def to_dict(self) -> Dict[str, Any]:
        """Конвертирует в словарь - БЕЗОПАСНАЯ ВЕРСИЯ"""
        result = {}
        ...
        try:
            ...
            # Минимальный работающий словарь
            result = {
    ...
    def update(self, **kwargs):
        ...
        for key, value in kwargs.items():
            if hasattr(self, key):
                # Специальная обработка для current_stage
                if key == "current_stage":
                    if isinstance(value, ConversationStage):
                        setattr(self, key, value)
                    elif isinstance(value, str):
                        # Преобразуем строку в ConversationStage
                        try:
    ...
    def get_symptom_text(self) -> str:
        """Возвращает текст всех симптомов"""
        return " ".join(self.symptom_texts)

    def get_normalized_symptoms(self) -> List[str]:
        """Возвращает нормализованные симптомы как список строк"""
        normalized = []
    ...
    def get_symptoms_for_processing(self) -> List[str]:
        """Возвращает симптомы в формате для обработки агентами"""
        result = []

        # Сначала пробуем normalized_symptoms (из symptom_normalizer)
        ...
        # Если нет normalized_symptoms, используем symptom_texts
