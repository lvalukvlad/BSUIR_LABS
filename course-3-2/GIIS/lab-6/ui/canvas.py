...
def draw_preview():
    ...
    if state.mode_lab == 1 and state.current_algorithm:
        draw_line_preview()
    elif state.mode_lab == 2 and state.current_algorithm:
        draw_shape_preview()
    elif state.mode_lab == 4:
...
def on_click(event):
    if state.current_algorithm is None and state.mode_lab not in [3, 4, 6]:
...
def on_move(event):
    if state.mode_lab == 4 and state.polygon_mode == 'build' and state.polygon_points:
        state.preview_end = screen_to_grid(event.x, event.y)
        redraw()
        return
    
    if state.mode_lab == 1 and state.current_algorithm and state.start_point and state.end_point is None and state.mode == "normal":
        state.preview_end = screen_to_grid(event.x, event.y)
        redraw()
...
def redraw():
    ...
    if state.mode_lab == 3 and state.lab_3_points:
